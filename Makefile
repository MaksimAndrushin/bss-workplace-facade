...
SERVICE_NAME=bss-workplace-facade
SERVICE_PATH=ozonmp/bss-workplace-facade
...
.PHONY: run
run:
	go run cmd/bss-workplace-facade/main.go
...
.generate-finalize-python:
	find pypkg/bss-workplace-facade -type d -exec touch {}/__init__.py \;
...
.build:
	go mod download && CGO_ENABLED=0  go build \
		...
		" \
		-o ./bin/bss-workplace-facade$(shell go env GOEXE) ./cmd/bss-workplace-facade/main.go
